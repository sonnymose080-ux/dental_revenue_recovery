...
export async function POST(request: Request) {
  try {
    ...
    const lead = {
      ...
      source: body.source ? String(body.source).trim() : 'webhook',
      status: 'new',
    ...
    const { data, error } = await supabase.from('leads').insert(lead).select('id,name,status,source,estimated_value,follow_up_at').single();
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
    await supabase.from('activity_log').insert({ clinic_id: DEMO_CLINIC_ID, lead_id: data.id, event_type: 'lead_created', description: `Lead created from ${lead.source}`, metadata: { source: lead.source } });
    return NextResponse.json({ ok: true, received: true, event: 'lead', demo: false, lead: data }, { status: 201 });
